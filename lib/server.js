...
function cleanup() {
  ...
  const incompleteScenarios = Object.entries(scenarioEntriesMap).filter(([scenario, entries]) => !!entries.length);
  console.log(incompleteScenarios);

  if (!incompleteScenarios.length) {
    process.exit(0);
  } else {
    console.log('There are still some scenarios that you need to handle.');
