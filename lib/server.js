...
server.playbackProxy = ({app, scenarioName, req, res}) => {
  ...
  if (!nextEntry) {
    return util.sendError(res, stripIndent`
      ...
      ${req.method} ${app.proxyTarget + req.url}
      ${req.body}
    `);
  }
  ...
  // Validate the path
  const urlPath = nextRequest.url;

  const errors = [];

  if (req.url !== urlPath) {
    errors.push(`expected a url of ${urlPath}, but received ${req.url}`);
  }
  ...
  if (req.body !== nextRequest.postData.text) {
    if (!req.body) {
      ...
      const nextBody = JSON.parse(nextRequest.postData.text);
      const receivedBody = JSON.parse(req.body);

      const writeableNextBody = app.specUtil.removeReadOnlyProps({
        body: nextBody,
        req: nextRequest
      });

      const writeableReceivedBody = app.specUtil.removeReadOnlyProps({
        body: receivedBody,
        req
      });

      // Build a string to easily show the differences
      const diffs = JsDiff.diffJson(writeableNextBody, writeableReceivedBody);
      if (diffs.length > 1) {
        ...
        errors.push('received different body than expected');
        errors.push(diffStr);
      }
    }
  }
    ...
    for (const [scenario, entries] of incompleteScenarios) {
      ...
      if (app.verbose) {
        ...
        entries.map(entry => {
          ...
          console.log(`\t${entry.request.method} ${urlPath}`);
        });
      }
    }
    process.exit(1);
