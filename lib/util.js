...
util.parseCommandLineArgs = defaults => {
  program
    ...
    .option('-s, --scenarios <items>', 'Comma-separated list of the scenaros to run', util.splitList)
...
util.splitList = function (items) {
  return items ? items.split(',') : [];
};

util.packageScenarioDir = () => {
  return path.join(__dirname, '..', 'scenarios');
};

util.sendError = (res, err, code = 400) => {
  console.error(err);
  res.statusCode = code;
  return res.end(err);
};
